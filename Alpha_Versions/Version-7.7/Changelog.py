...
def Changelog(screen, down, x2):
    Version = [["dev-0.0", "---{2022-11-15}---", "add: iso-codeline", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "", "Here is a not added hidden achievement lol"],
               ...
               ["alpha-5.d", "---{2023-11-20}---", "change: 'Games played' now 'Games won'", "fix: Game Over now without 0", "change: losing lv.3 gets you back to lv.1"],
               ...
               ["alpha-7.1", "---{2024-01-17}---", "add: more oop /w 5pelf A.K.A King Canute of England"],
               ["alpha-7.2", "---{2024-02-24}---", "fix: no display of values on the pygame window"],
               ["alpha-7.3", "---{2024-02-25}---", "add: saving in a file", "fix: non-integer values as size", "fix: 'noo' output on the saveprint"],
               ["alpha-7.4", "---{2024-02-25}---", "change: 42 achievement requires now 420", "fix: crash after getting the death value", "add: Achievements for Lv.1", "add: saving Achievements"],
               ["alpha-7.5", "---{2024-02-25}---", "add: achievements for Lv.2"],
               ["alpha-7.6", "---{2024-03-29}---", "fix: Changelog not scrolling down", "fix: Lv.3 afk reset", "fix: Lv.3 win instantly after hitting 1M"],
               ["alpha-7.7", "---{2024-07-06}---", "fix: Different display Sizes not showing Menu, Saves and Changelog correctly", "fix: Spelling mistakes in Changelog"]
               ]
    screen.fill([0, 0, 0])
    deve = 1
    alp = 0
    line = 0
    T = [None, 100*x2, "Changelog", True, 255, 255, 255, screen.get_width()/2, screen.get_height()/16]
    screen = Texts(T,screen,x2)
    for dev in range(0, len(Version)):
        sub = Version[len(Version)-dev-1]
        if screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2 > screen.get_height()/8 + 50*x2 and screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2 < screen.get_height() - 10*x2:
            if sub[0] == "--------------------------------------   Early development   --------------------------------------":
                if screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2 < screen.get_height() - 10*x2:
                    T = [None, 50*x2, sub[0], True, 255, 255, 127, screen.get_width()/2, screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2]
                    screen = Texts(T,screen,x2)
            if sub[0] != "--------------------------------------   Early development   --------------------------------------":
                T = [None, 50*x2, sub[0], True, 255, 255, 0, screen.get_width()/2, screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2]
            screen = Texts(T,screen,x2)
        if len(sub) > 0:
            for deve in range(1, len(sub)):
                if deve > 1:
                    line = line + 1
                if screen.get_height()/8 + 40*x2 + 60*dev*x2 + 30*(line+2)*x2 - down*25*x2 > screen.get_height()/8 + 50*x2 and screen.get_height()/8 + 40*x2 + 60*dev*x2 + 30*(line+2)*x2 - down*25*x2 < screen.get_height() - 10*x2:
                    T = [None, 25*x2, sub[deve], True, 127, 255, 0, screen.get_width()/2, screen.get_height()/8 + 40*x2 + 60*dev*x2 + 30*(line+2)*x2 - down*25*x2]
                    if deve == 1:
                        T = [None, 25*x2, sub[deve], True, 191, 255, 63, screen.get_width()/2, screen.get_height()/8 + 40*x2 + 60*dev*x2 + 30*(line+2)*x2 - down*25*x2]
                    screen = Texts(T,screen,x2)
        if sub[0] == "--------------------------------------   Early development   --------------------------------------" and screen.get_height()/8 + 45*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2 > screen.get_height()/8 + 50*x2:
            T = [None, 75*x2, "Alpha development", True, 255, 255, 127, screen.get_width()/2, screen.get_height()/8 + 20*x2]
            alp = 1
            screen = Texts(T,screen,x2)
    if screen.get_height()/8 + 25*x2 + 60*dev*x2 + 30*(line+1)*x2 - down*25*x2 > screen.get_height()/8 and alp == 0:
        T = [None, 75*x2, "Early development", True, 255, 255, 127, screen.get_width()/2, screen.get_height()/8 + 20*x2]
        screen = Texts(T,screen,x2)
    time.sleep(0.04)
    return screen, down
