...
def Calcul1000(cent):
    ...
    while deci >= 10:
        centi = centi + 1
        deci = deci - 10
    MIL, MIL2, DECI, CENTI = ["", "M", "B", "T", "Q", "q", "S", "s", "O", "N"], ["", "U", "D", "T", "Q", "q", "S", "s",
                                                                                 "O", "N"], ["", "De", "Vi", "Tr", "Qu",
                                                                                             "qu", "Se", "se", "Ok",
                                                                                             "No"], ["", "Cen", "Duc",
                                                                                                     "Tre", "Qua",
...
def Ptsview(c, extra, a, notthird, goal, A, nuxt, ten, screen, tick, b):
    global nah
    cent = int(c)
    lten = ten
...
def PV_ani(b, extra, notthird, goal, A, tick, nixt, stoppy, a, ten, screen):
    if goal == 10:
        if stoppy == True:
            c = b
        else:
            ...
            if c != c1:
                c = int(c1 * 100) / 100
        Ptsview(c, extra, a, notthird, goal, A, nixt, ten, screen, tick, b)
        if tick == 96:
            stoppy = True
    if goal == 1000:
        if stoppy == True:
            ...
            if c != c1:
                c = int(c1 * 100) / 100
        Ptsview(c, zu, x, notthird, goal, poss, noxt, ten, screen, tick, b)
        if tick == 96:
            stoppy = True
    if goal == 1000000:
        if stoppy == True:
            ...
            if c != c1:
                c = int(c1 * 100) / 100
        Ptsview(c, pre, nex, notthird, goal, rest0, naxt, ten, screen, tick, b)
        if tick == 96:
            stoppy = True