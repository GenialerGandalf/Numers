...
def Numers_Forward(a1, n1, L2, a, go, noxt, naxt, nixt, q, total, b):
    pre, zu, nex, x, poss, rest, rest1, rest2, rest0 = "", "", "", "", 0, 0, 0, 0, 0
    if L2 > 0 and total < 1000000:
        n1, L2, naxt, total, goal, pre, nex, rest1, rest2, rest0 = NF3(L2, go, naxt)
    elif n1 > 0 and L2 == 0:
        L2, n1, noxt, q, goal, zu, x, poss, rest = NF2(n1, L2, go, noxt)
    elif n1 == 0 and L2 == 0:
        n1, a1, a, b, nixt, goal = NF1(a1, a, go, nixt)
    return a1, n1, L2, a, go, b, noxt, naxt, nixt, q, total, goal, pre, zu, nex, x, poss, rest, rest1, rest2, rest0
...
def Savestates(screen, go, tog, games, save, qhs, bhs, hs):
    ...
    screen = Texts(T,screen,x2)
    T = [None, 25,
         "Date: " + str(Save[0])[0:4] + "-" + str(Save[0])[4:6] + "-" + str(Save[0])[6:] + " " + str(hour) + ":" + str(
             rminu) + ":" + str(rsec), True, 255, 255, 255, screen.get_width() / 4 + 50, screen.get_height() / 32 * 13]
    screen = Texts(T,screen,x2)
...
while go != "n":
    ...
    if game == True:
        ...
        if tick == 100:
            ...
            tack = tack + 1
            if total < 999999:
                a1, n1, L2, a, go, b, noxt, naxt, nixt, q, total, goal, pre, zu, nex, x, poss, rest, rest1, rest2, rest0 = Numers_Forward(a1, n1, L2, a, go, noxt, naxt, nixt, q, total, b)
            if total >= 1000000:
                screen.fill([0, 0, 0])
                goal = 0
