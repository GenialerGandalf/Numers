...
stoppy, ten = False, -1
Ach = []
Choice2 = [255, 127, 63, 0, 0, 0, 0, 0]
total, extra, pre, zu, poss, rest, rest1, rest2, rest0, c, high = 0, "", "", "", 0, 0, 0, 0, 0, 0, -1
screen.fill([0, 0, 0])
pygame.display.flip()
...
while go != "n":
    ...
    if game1 == True:
        if menu == True:
            screen.fill([0, 0, 0])
            if change != True and achi != True and save != True:
                T = [None, 150*x2, "Select Story", True, 255, 127, 0, screen.get_width() / 2, screen.get_height() / 10]
                screen = Texts(T,screen,x2)
                if high >-3.5:
                    T = [None, 100*x2, "Numers Nothing", True, Choice2[0], Choice2[0]/1.75, Choice2[0]/4, screen.get_width() / 2,
                        screen.get_height() / 8 * (5+high)]
                    screen = Texts(T,screen,x2)
                if high >-4.5:    
                    T = [None, 100*x2, "Numers Forward", True, Choice2[1], Choice2[1]/1.5, Choice2[1]/4, screen.get_width() / 2,
                        screen.get_height() / 8 * (6+high)]
                if high >-5.5:
                    screen = Texts(T,screen,x2)
                    T = [None, 100*x2, "Numers Enemies", True, Choice2[2], Choice2[2]/1.25, Choice2[2]/4, screen.get_width() / 2,
                        screen.get_height() / 8 * (7+high)]
                    screen = Texts(T,screen,x2)
                if hs >= 1000000:
                    if high >-6.5:
                        T = [None, 100*x2, "Numers Dungeons", True, Choice2[3], Choice2[3], Choice2[3]/4, screen.get_width() / 2,
                            screen.get_height() / 8 * (8+high)]
                        screen = Texts(T,screen,x2)
                    if high >-7.5:
                        T = [None, 100*x2, "Numers Civilisation", True, Choice2[4]/1.25, Choice2[4], Choice2[4]/4, screen.get_width() / 2,
                            screen.get_height() / 8 * (9+high)]
                        screen = Texts(T,screen,x2)
                    if high >-8.5:
                        T = [None, 100*x2, "Numers Beginnings", True, Choice2[5]/1.5, Choice2[5], Choice2[5]/6, screen.get_width() / 2,
                            screen.get_height() / 8 * (10+high)]
                        screen = Texts(T,screen,x2)
                        T = [None, 100*x2, "Numers Adventures", True, Choice2[6]/2, Choice2[6], Choice2[6]/8, screen.get_width() / 2,
                            screen.get_height() / 8 * (11+high)]
                        screen = Texts(T,screen,x2)
                        T = [None, 100*x2, "Numers Spaceworld", True, Choice2[7]/4, Choice2[7], Choice2[7]/16, screen.get_width() / 2,
                            screen.get_height() / 8 * (12+high)]
                screen = Texts(T,screen,x2)
            go = Return(go)
            if go == "y" and Choice2[1] == 255:
                menu = False
                game = True
            elif go == "y" and Choice2[0] == 255:
                print("not available yet")
            elif go == "y" and Choice2[2] == 255:
                vsgame()
                go = "y"
            elif go == "y" and Choice[3] == 255:
                change = True
                ...
                if high < -1:
                    high = high + 0.04
                if Choice2[1] == 255 and high >= -1.5 and high <= -0.5:
                    Choice2 = [255, 127, 63, 0, 0, 0, 0, 0]
                elif Choice2[2] == 255 and high >= -2.5 and high <= -1.5:
                    Choice2 = [127, 255, 127, 63, 0, 0, 0, 0]
                elif Choice2[3] == 255 and high >= -3.5 and high <= -2.5:
                    Choice2 = [63, 127, 255, 127, 63, 0, 0, 0]
                elif Choice2[4] == 255 and high >= -4.5 and high <= -3.5:
                    Choice2 = [0, 63, 127, 255, 127, 63, 0, 0]
                elif Choice2[5] == 255 and high >= -5.5 and high <= -4.5:
                    Choice2 = [0, 0, 63, 127, 255, 127, 63, 0]
                elif Choice2[6] == 255 and high >= -6.5 and high <= -5.5:
                    Choice2 = [0, 0, 0, 63, 127, 255, 127, 0]
                elif Choice2[7] == 255 and high >= -7.5 and high <= -6.5:
                    Choice2 = [0, 0, 0, 0, 63, 127, 255, 127]
            elif go == "down":
                if hs < 1000000:
                    if high > -2:
                        high = high - 0.04
                    ...
                    if high > -7.9:
                        high = high - 0.04
                if Choice2[0] == 255 and high <= -1.5 and high >= -2.5:
                    Choice2 = [127, 255, 127, 63, 0, 0, 0, 0]
                elif Choice2[1] == 255 and hs >= 1000000 and high <= -2.5 and high >= -3.5:
                    Choice2 = [63, 127, 255, 127, 63, 0, 0, 0]
                elif Choice2[2] == 255 and high <= -3.5 and high >= -4.5:
                    Choice2 = [0, 63, 127, 255, 127, 63, 0, 0]
                elif Choice2[3] == 255 and high <= -4.5 and high >= -5.5:
                    Choice2 = [0, 0, 63, 127, 255, 127, 63, 0]
                elif Choice2[4] == 255 and high <= -5.5 and high >= -6.5:
                    Choice2 = [0, 0, 0, 63, 127, 255, 127, 63]
                elif Choice2[5] == 255 and high <= -6.5 and high >= -7.5:
                    Choice2 = [0, 0, 0, 0, 63, 127, 255, 127]
                elif Choice2[6] == 255 and high <= -7.5 and high >= -8.5:
                    Choice2 = [0, 0, 0, 0, 0, 63, 127, 255]
            elif go == "esc":
                if game1 == True:
                    game1 = False
                screen.fill([0, 0, 0])
